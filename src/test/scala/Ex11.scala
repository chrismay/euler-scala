...
class Ex11 extends FunSuite with ShouldMatchers {
  ...
  test("left-to-right quadsFromLine works right")(
    lrQuadsFromLine(List(1, 2, 3, 4, 5, 6), List.empty) should equal(
      List(List(1, 2, 3, 4),
        List(2, 3, 4, 5),
        List(3, 4, 5, 6))))
  ...
  def rotate90Deg(grid: List[List[Int]]): List[List[Int]] = {
    grid.head.zipWithIndex.map((t) => {
      grid.map((row: List[Int]) => row(t._2)).reverse
    })
  }
